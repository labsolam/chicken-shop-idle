...
- [x] Add manager state to `GameState`:
  ```
  managers: {
  ...
  ```
- [x] Add `lastOnlineTimestamp: number` for offline earnings (written on each save; `lastUpdateTimestamp` updates every tick and is used for delta calculations — they serve different purposes)
- [x] Add `revenueTracker: { recentRevenueCents, trackerElapsedMs, lastComputedRatePerMs }` for base rate tracking
- [x] Add `totalChickensBought: number` lifetime stat (used for Buyer Bob analytics in UI — shows "chickens auto-purchased" count)
- [x] Update `createInitialState()` and save/load

### Step 2: Manager hiring and upgrading

- [x] Create `src/engine/managers.ts` with `hireManager()`, `upgradeManager()`, `getManagerUpgradeCost()`
- [x] Manager unlock checks based on `totalRevenueCents`
- [x] Write tests for hire, upgrade, cost scaling, and unlock conditions

### Step 3: Manager processing in tick()

- [x] Add manager timer processing to `tick()`:
...
- [x] Write tests for manager automation at various levels

### Step 4: Revenue tracking and offline earnings

- [x] Implement rolling revenue tracker in `tick()` (track revenue over 60s windows)
- [x] Replace the no-op `calculateOfflineEarnings()` with real implementation
- [x] Offline earnings formula: `baseRate × clampedDuration × 0.30`
- [x] Write tests for revenue tracking, offline earnings calculation

### Step 5: Customer Tips

- [x] Add `tipsLevel: number` to GameState
- [x] Add `"tips"` to UpgradeType
- [x] Tip upgrade cost lookup table (hand-tuned from doc 003, converted to cents):
  ```
...
- [x] Implement per-sale tip check in sell completion within `tick()`
...
- [x] Write tests for tip chance, tip bonus, and integration

### Step 6: Click bonuses

- [x] When a manager is active and player clicks the same action, grant 10% bonus
- [x] 1s internal cooldown per action type
- [x] Add `lastClickTimestamps: { buy, cook, sell }` to GameState
- [x] Write tests

### Step 7: UI updates

- [x] Manager panel: show available/locked managers, hire buttons, upgrade buttons
- [x] Income/second display in header
- [x] "Auto" indicators on Buy/Cook/Sell buttons when manager is active
- [x] Welcome-back screen showing offline earnings
- [x] Tips upgrade in the Value upgrade section
...
> **Note:** E2E tests were not updated in this plan. Playwright browser binaries were unavailable in the implementation environment. Existing e2e tests pass code-wise; Phase 2 UI additions are covered by unit tests in `tests/ui/render.test.ts`. E2E coverage for Phase 2 features is deferred to a follow-up plan.

### Step 9: Run full check

- [x] `npm run check` — all 312 unit tests pass, lint clean, format clean
- [ ] `npm run test:e2e` — skipped (Playwright browsers unavailable in environment)
...
> **Manager speed formula correction:** Doc 004 says `interval = baseInterval × (1 - speedBonus)`, but this breaks at Level 4 (+100% speed → interval = 0) and goes negative at Level 5+. Use the reciprocal formula `interval = baseInterval / (1 + speedBonus)` instead. At +25% speed: `base / 1.25` (20% faster). At +100%: `base / 2` (twice as fast). At +2000%: `base / 21` (~21× faster). This is the standard idle game approach and produces sensible values at all levels. Decision record `docs/decisions/013-manager-speed-formula.md` documents this override.

## Outcome

Implemented all core Phase 2 features in a single commit:

- **3 manager NPCs** fully wired end-to-end: hire/upgrade/click-bonus in engine, timer automation in tick(), hire/upgrade UI in render.ts + index.html + main.ts
- **Revenue tracker** (rolling 60s window) tracks `lastComputedRatePerMs` for offline earnings and income/sec display
- **Offline earnings** replaced the no-op: `baseRatePerMs × elapsed × 0.30`, capped at 4h, uses `lastOnlineTimestamp` (injected at save time) to avoid tick-time contamination
- **Customer Tips** (10 levels) with per-sale RNG check; `tick()` accepts deterministic `rng` parameter for testability
- **Click bonuses** with 1s cooldown per action type
- **312 unit tests** passing; 13 test files covering all Phase 2 behaviour
- **Decision 013** documents the reciprocal manager speed formula override
- E2E test coverage for Phase 2 UI deferred (browser binaries unavailable in implementation environment)
